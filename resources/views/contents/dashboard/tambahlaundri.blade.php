        ...
        <div class="row">
            ...
            <div class="col-xl-10">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('laundri') }}">Kembali</a></li>
                        <li class="breadcrumb-item active" aria-current="page">Tambah Laundri</li>
                    </ol>
                </nav>

                <div class="card border-0">
                    <div class="card-body">
                        <form action="{{ route('posttambahlaundri') }}" class="row" autocomplete="off" method="POST">
                            @csrf
                            <div class="row">
                                {{-- Penyewa --}}
                                <div class="col-lg-6 mb-3">
                                    ...
                                    <select class="form-select form-select-2 @error('penyewa') is-invalid @enderror"
                                        ...
                                        <option>Cari Nama Penyewa</option>
                                        @foreach ($penyewa as $row)
                                            <option value="{{ $row->penyewa_id }}"
                                                {{ old('penyewa') == $row->penyewa_id ? 'selected' : '' }}>
                                                {{ $row->namalengkap }} | Nomor Kamar: {{ $row->nomor_kamar }}</option>
                                        @endforeach
                                    </select>
                                    @error('penyewa')
                                ...
                                {{-- Jumlah Kilo --}}
                                <div class="col-lg-6 mb-3">
                                    <label for="kiloan" class="form-label fw-bold">Jumlah Kilo <sup
                                            class="text-danger">*</sup></label>
